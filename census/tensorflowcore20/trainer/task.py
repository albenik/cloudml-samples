...
class EvaluationRunHook(tf.estimator.SessionRunHook):
  ...
  def __init__(self,
    ...
    with graph.as_default():
      ...
      self._summary_op = tf.compat.v1.summary.merge([
          tf.compat.v1.summary.scalar(name, value_op)
      ...
      self._gs = tf.compat.v1.train.get_or_create_global_step()
  ...
  def _run_eval(self):
    ...
    with tf.compat.v1.Session(graph=self._graph) as session:
      ...
      tf.compat.v1.train.start_queue_runners(coord=coord, sess=session)
      train_step = session.run(self._gs)

      tf.compat.v1.logging.info('Starting Evaluation For Step: {}'.format(train_step))
      with coord.stop_on_exception():
        ...
        while not coord.should_stop() and (self._eval_steps is None or
          ...
          if eval_step % 100 == 0:
            tf.compat.v1.logging.info('On Evaluation Step: {}'.format(eval_step))
          eval_step += 1

      # Write the summaries
      self._file_writer.add_summary(summaries, global_step=train_step)
      self._file_writer.flush()
      tf.compat.v1.logging.info(final_values)
...
def run(target, cluster_spec, is_chief, args):
  ...
  if is_chief:
    tf.compat.v1.logging.info('Created DNN hidden units {}'.format(hidden_units))
  ...
  with tf.Graph().as_default():
    ...
    with tf.compat.v1.train.MonitoredTrainingSession(master=target,
                                           is_chief=is_chief,
...
def build_and_run_exports(latest, job_dir, serving_input_fn, hidden_units):
  ...
  # Create exporter.
  exporter = tf.compat.v1.saved_model.builder.SavedModelBuilder(
  ...
  with prediction_graph.as_default():
    ...
    inputs_info = {
        name: tf.compat.v1.saved_model.utils.build_tensor_info(tensor)
  ...
  with tf.compat.v1.Session(graph=prediction_graph) as session:
    session.run([tf.compat.v1.local_variables_initializer(), tf.compat.v1.tables_initializer()])
...
if __name__ == '__main__':
  ...
  os.environ['TF_CPP_MIN_LOG_LEVEL'] = str(
    tf.compat.v1.logging.__dict__[args.verbosity] / 10)
