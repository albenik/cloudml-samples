...
def model_fn(mode,
  ...
  embed_initializer = tf.compat.v1.initializers.truncated_normal(
      stddev=(1.0 / tf.sqrt(float(embedding_size))))

  with tf.compat.v1.variable_scope('embeddings', initializer=embed_initializer):
    ...
    for col, vals in CATEGORICAL_COLS:
      ...
      embeddings = tf.compat.v1.get_variable(col, shape=[bucket_size, embedding_size])
      ...
      features[col] = tf.nn.embedding_lookup(params=embeddings, ids=indices)

  for col in CONTINUOUS_COLS:
    # Give continuous columns an extra trivial dimension
    # So they can be concatenated with embedding tensors
    features[col] = tf.expand_dims(tf.cast(features[col], dtype=tf.float32), -1)
  ...
  for layer_size in hidden_units:
    curr_layer = tf.compat.v1.layers.dense(
        curr_layer,
        layer_size,
  ...
  if mode in (PREDICT, EVAL):
    ...
    predicted_indices = tf.argmax(input=probabilities, axis=1)
  ...
  if mode == PREDICT:
    ...
    return {
        ...
        'confidence': tf.reduce_max(input_tensor=probabilities, axis=1)
    }
  ...
  if mode == TRAIN:
    ...
    train_op = tf.compat.v1.train.FtrlOptimizer(
  ...
  if mode == EVAL:
    ...
    return {
        ...
        'auroc': tf.compat.v1.metrics.auc(labels_one_hot, probabilities)
    }
...
def csv_serving_input_fn(default_batch_size=None):
  ...
  """
  csv_row = tf.compat.v1.placeholder(shape=[default_batch_size], dtype=tf.string)
...
def example_serving_input_fn(default_batch_size=None):
  ...
  for feat in CONTINUOUS_COLS:
    feature_spec[feat] = tf.io.FixedLenFeature(shape=[], dtype=tf.int64)

  for feat, _ in CATEGORICAL_COLS:
    feature_spec[feat] = tf.io.FixedLenFeature(shape=[], dtype=tf.string)

  example_bytestring = tf.compat.v1.placeholder(
      shape=[default_batch_size],
      dtype=tf.string,
  )
  features = tf.io.parse_example(serialized=example_bytestring, features=feature_spec)
...
def json_serving_input_fn(default_batch_size=None):
  ...
  inputs = {}
  for feat in CONTINUOUS_COLS:
    inputs[feat] = tf.compat.v1.placeholder(shape=[default_batch_size], dtype=tf.float32)

  for feat, _ in CATEGORICAL_COLS:
    inputs[feat] = tf.compat.v1.placeholder(shape=[default_batch_size], dtype=tf.string)
...
def _decode_csv(line):
  ...
  columns = tf.io.decode_csv(records=line, record_defaults=CSV_COLUMN_DEFAULTS)
