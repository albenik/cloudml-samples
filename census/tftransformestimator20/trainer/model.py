...
def vocabulary_file_by_name(working_dir, key):
  """Retrieves the path of the vocabulary created by tf transform and named after the feature_column.

  Args:
      working_dir (str): Directory pointed by the tf transform pipeline
      key (str): Name of the feature_column

  Returns:
    The path to the vocabulary
  """
  return os.path.join(working_dir, transform_fn_io.TRANSFORM_FN_DIR, 'assets',
                      key)


def vocabulary_size_by_name(working_dir, key):
  """Retrieves the size of the vocabulary created by tf transform and named after the feature_column.
  ...
  with tf.io.gfile.GFile(vocabulary, 'r') as f:
...
def _make_training_input_fn(tft_working_dir,
  ...
  def parser(record):
    ...
    parsed = tf.io.parse_single_example(serialized=record, features=transformed_feature_spec)
